...
import styles from "./detail.module.css"; // CSS Modules 사용
...
export default function SoulDetailPage() {
  ...
  return (
    <div className={styles.container}>
      ...
      {soul.locationImage && (
        <div className={styles.imageWrapper}>
          <img src={soul.locationImage} alt="위치 이미지" className={styles.image} />
        </div>
      )}
        ...
        <div className={styles.section}>
          ...
          <ul className={styles.imageList}>
            {soul.wearingShotImages.map((img, index) => (
              <li key={index}>
                <img src={img} alt={`착용샷 ${index + 1}`} className={styles.image} />
              </li>
            ))}
          </ul>
        </div>
